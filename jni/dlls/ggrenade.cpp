...
void CGrenade::Explode3(TraceResult *pTrace, int bitsDamageType)
{
	float flRndSound; // sound randomizer
	//float damage;
UTIL_ScreenShake(pTrace->vecEndPos, 25, 150, 1, 500);
...
RadiusDamage(pev, pevOwner, 200, CLASS_NONE, bitsDamageType);
...
//pb stuff
void CGrenade::BounceTouchExplode(CBaseEntity *pOther)
{
...
//moly for wp
CGrenade* CGrenade::ShootMolotov(entvars_t* pevOwner, Vector vecStart, Vector vecVelocity, float time, unsigned short usEvent)
{
...
pGrenade->SetTouch(&CGrenade::BounceTouch);
	...
	SET_MODEL(ENT(pGrenade->pev), "models/w_wp_smoke.mdl");
...
//medkit test
CGrenade* CGrenade::ShootMedkit(entvars_t* pevOwner, Vector vecStart, Vector vecVelocity, float time, unsigned short usEvent)
{
	CGrenade* pGrenade = CreateClassPtr<CGrenade>();
	pGrenade->Spawn();
...
pGrenade->SetTouch(&CGrenade::BounceTouch); //touch break

	pGrenade->pev->dmgtime = gpGlobals->time + time;

pGrenade->SetThink(&CGrenade::Med_TumbleThink);
	...
	SET_MODEL(ENT(pGrenade->pev), "models/w_medkit.mdl");
	pGrenade->pev->dmg = 35.0f;
...
void CGrenade::Med_BounceTouch(CBaseEntity* pOther)
{
	// don't hit the guy that launched this grenade
	if (pOther->edict() == pev->owner)
		return;
...
void CGrenade::Med_Detonate()
{
	...
	vecSpot = pev->origin + Vector(0, 0, 8);
	UTIL_TraceLine(vecSpot, vecSpot + Vector(0, 0, -40), ignore_monsters, ENT(pev), &tr);
	Med_Explode(&tr, DMG_EXPLOSION);
}


void CGrenade::Med_Explode(TraceResult* pTrace, int bitsDamageType)
{
	pev->model = iStringNull; // invisible
	pev->solid = SOLID_NOT; // intangible
	pev->takedamage = DAMAGE_NO;
	...
	for (int i = -num; i <= num; i++)	//i= -5 10�ݻ���
	{
		for (int j = -num; j <= num; j++) //��10��
		{
			...
			Create("medkit_effect", origin2, pTrace->vecPlaneNormal, pev->owner);
...
//pb stuff 
CGrenade *CGrenade::ShootExplode(entvars_t *pevOwner, Vector vecStart, Vector vecVelocity, float time, int iTeam, unsigned short usEvent)
{
	CGrenade *pGrenade = CreateClassPtr<CGrenade>();
	pGrenade->Spawn();
...
Vector vecDirection = (pGrenade->pev->velocity).Normalize();

	pGrenade->pev->velocity = vecVelocity + vecDirection + vecArrowAngle;
	pGrenade->pev->angles = pevOwner->angles;
	pGrenade->pev->owner = ENT(pevOwner);

	pGrenade->m_usEvent = usEvent;

	pGrenade->SetTouch(&CGrenade::BounceTouchExplode);


	pGrenade->pev->nextthink = gpGlobals->time + 0.0001f;

	pGrenade->pev->sequence = RANDOM_LONG(3, 6);

	pGrenade->pev->gravity = 0.0005f;
	pGrenade->pev->friction = 0.005f;

	pGrenade->m_iTeam = iTeam;

	SET_MODEL(ENT(pGrenade->pev), "models/arrow.mdl");
	//pGrenade->pev->dmg = 100.0f;
...
CGrenade *CGrenade::ShootLpg(entvars_t *pevOwner, Vector vecStart, Vector vecVelocity, float time, int iTeam, unsigned short usEvent)
{
	CGrenade *pGrenade = CreateClassPtr<CGrenade>();
	pGrenade->Spawn();
	...
	SET_MODEL(ENT(pGrenade->pev), "models/w_gasbomb_melon.mdl");
	pGrenade->pev->dmg = 100.0f;
