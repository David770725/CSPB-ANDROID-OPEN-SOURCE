...
LINK_ENTITY_TO_CLASS(weapon_dual_knife, CDual_knife)
...
void CDual_knife::Precache(void)
{
...
PRECACHE_SOUND("weapons/dual_knife_draw.wav");
	...
	m_usDual_knife = PRECACHE_EVENT(1, "events/dual_knife.sc");
}

int CDual_knife::GetItemInfo(ItemInfo *p)
{
...
BOOL CDual_knife::Deploy(void)
{
	m_fMaxSpeed = 250;
	m_iSwing = 0;
	
		
return DefaultDeploy("models/billflx/v_dual_knife.mdl", "models/p_dual_knife.mdl", KNIFE_DRAW, "knuckle", UseDecrement() != FALSE);


}


void CDual_knife::QuickDeploy()
{
...
void CDual_knife::Holster(int skiplocal)
{
	m_pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + 0.5;
SetThink(nullptr);
}

void CDual_knife::WeaponAnimation(int iAnimation)
{
	int flags;
	...
	PLAYBACK_EVENT_FULL(flags, ENT(m_pPlayer->pev), m_usDual_knife, 0, (float *)&g_vecZero, (float *)&g_vecZero, 0, 0, iAnimation, 2, 3, 4);
}

void FindHullIntersectionDualknife(const Vector &vecSrc, TraceResult &tr, float *pflMins, float *pfkMaxs, edict_t *pEntity)
{
	TraceResult trTemp;
...
void CDual_knife::PrimaryAttack(void)
{
	...
	SetThink(&CDual_knife::DelayedPrimaryAttack);
...
void CDual_knife::DelayedPrimaryAttack()
{
	SetThink(&CDual_knife::DelayedPrimaryAttack2);
	pev->nextthink = gpGlobals->time + 0.22;
	...
	if (tr.flFraction >= 1)
	{
		...
		if (tr.flFraction < 1)
		{
			...
			if (!pHit || pHit->IsBSPModel())
				FindHullIntersectionDualknife(vecSrc, tr, VEC_DUCK_HULL_MIN, VEC_DUCK_HULL_MAX, ENT(m_pPlayer->pev));
...
void CDual_knife::DelayedPrimaryAttack2()
{
...
void CDual_knife::SetPlayerShieldAnim(void)
{
	;
}

void CDual_knife::ResetPlayerShieldAnim(void)
{
	;
}

bool CDual_knife::ShieldSecondaryFire(int up_anim, int down_anim)
{
	;
}

void CDual_knife::SecondaryAttack(void)
{
	#ifndef CLIENT_DLL
	m_pPlayer->SetAnimation(PLAYER_ATTACK1);
#endif
	...
	SetThink(&CDual_knife::DelayedSecondaryAttack);
	pev->nextthink = gpGlobals->time + 0.22;
...
void CDual_knife::DelayedSecondaryAttack()
{

SetThink(&CDual_knife::DelayedSecondaryAttack2);
	pev->nextthink = gpGlobals->time + 0.22;
...
void CDual_knife::DelayedSecondaryAttack2()
{

SetThink(&CDual_knife::DelayedSecondaryAttack3);
...
void CDual_knife::DelayedSecondaryAttack3()
{
...
void CDual_knife::Smack(void)
{
	;
}

void CDual_knife::SwingAgain(void)
{
	Swing(FALSE);
}

void CDual_knife::WeaponIdle(void)
{
	ResetEmptySound();
...
int CDual_knife::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
...
int CDual_knife::Stab(int fFirst)
{
	;
}
