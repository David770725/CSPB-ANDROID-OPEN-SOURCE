...
LINK_ENTITY_TO_CLASS(weapon_amok, CAmok)
...
void CAmok::Precache(void)
{
PRECACHE_MODEL("models/billflx/v_amok_kukri.mdl");
...
PRECACHE_SOUND("weapons/amok_kukri_draw.wav");


	m_usAmok = PRECACHE_EVENT(1, "events/Amok.sc");
}

int CAmok::GetItemInfo(ItemInfo *p)
{
...
BOOL CAmok::Deploy(void)
{
	m_fMaxSpeed = 250;
	m_iSwing = 0;
	
		
return DefaultDeploy("models/billflx/v_amok_kukri.mdl", "models/p_amok_kukri.mdl", KNIFE_DRAW, "kukri", 0);

}

void CAmok::QuickDeploy()
{
...
void CAmok::Holster(int skiplocal)
{
...
void CAmok::WeaponAnimation(int iAnimation)
{
	int flags;
	...
	PLAYBACK_EVENT_FULL(flags, ENT(m_pPlayer->pev), m_usAmok, 0, (float *)&g_vecZero, (float *)&g_vecZero, 0, 0, iAnimation, 2, 3, 4);
}

void FindHullIntersectionAmok(const Vector &vecSrc, TraceResult &tr, float *pflMins, float *pfkMaxs, edict_t *pEntity)
{
	TraceResult trTemp;
...
void CAmok::PrimaryAttack(void)
{
	...
	SetThink(&CAmok::DelayedPrimaryAttack);
...
void CAmok::DelayedPrimaryAttack()
{
	BOOL fDidHit = FALSE;
...
void CAmok::SetPlayerShieldAnim(void)
{
	;
}

void CAmok::ResetPlayerShieldAnim(void)
{
	;
}

bool CAmok::ShieldSecondaryFire(int up_anim, int down_anim)
{
	;
}

void CAmok::SecondaryAttack(void)
{
	#ifndef CLIENT_DLL
	...
	SetThink(&CAmok::DelayedSecondaryAttack);
...
void CAmok::DelayedSecondaryAttack()
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
...
void CAmok::Smack(void)
{
	;
}

void CAmok::SwingAgain(void)
{
	Swing(TRUE);
}

void CAmok::WeaponIdle(void)
{
	/*ResetEmptySound();
...
int CAmok::Swing(int fFirst)
{
	;
}

int CAmok::Stab(int fFirst)
{
	;
}
