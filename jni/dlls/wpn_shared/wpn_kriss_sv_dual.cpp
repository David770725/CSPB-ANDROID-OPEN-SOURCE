...
LINK_ENTITY_TO_CLASS(weapon_kriss_sv_dual, CKriss_sv_dual)

void CKriss_sv_dual::Spawn(void)
{
	pev->classname = MAKE_STRING("weapon_kriss_sv_dual");
...
#ifndef CLIENT_DLL
void CKriss_sv_dual::AttachToPlayer(CBasePlayer *pPlayer)
{
	...
	while (!m_pLink)
	{
		for (int i = 0; i < MAX_ITEM_TYPES; ++i)
		{
			...
			while (pItem != NULL)
			{
				if (!Q_stricmp(pItem->pszName(), "weapon_kriss_sv"))
				{
					m_pLink = pItem;
		...
		if (!m_pLink)
			pPlayer->GiveNamedItem("weapon_kriss_sv");
...
void CKriss_sv_dual::ItemPostFrame()
{
	m_iSwing = 0;
	if (m_pLink)
		m_pLink->m_iSwing = 1;
...
void CKriss_sv_dual::Precache(void)
{
	
PRECACHE_MODEL("models/billflx/v_kriss_sv_2.mdl");

	PRECACHE_MODEL("models/p_kriss_sv_2.mdl");
...
int CKriss_sv_dual::GetItemInfo(ItemInfo *p)
{
...
BOOL CKriss_sv_dual::Deploy(void)
{
...
m_flAccuracy = 0.2;
	...
	if (m_iSwing)
	{
 DefaultDeploy("models/billflx/v_kriss_sv_2.mdl", "models/p_kriss_sv_2.mdl", SCARH_DRAW, "dualpistols", 0);
	}
...
void CKriss_sv_dual::QuickDeploy()
{
...
void CKriss_sv_dual::PrimaryAttack(void)
{
...
void CKriss_sv_dual::Kriss_sv_dualFire(float flSpread, float flCycleTime, BOOL fUseAutoAim)
{
	m_bDelayFire = true;
	m_iShotsFired++;
...
float CKriss_sv_dual::GetDamage()
{
#ifndef CLIENT_DLL
...
void CKriss_sv_dual::WeaponIdle(void)
{
	ResetEmptySound();
