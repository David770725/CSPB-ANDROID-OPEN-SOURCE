...
//ak47_damage = CVAR_GET_POINTER("ak47_damage");

//cvar_t ak47_damage = { "ak47_damage", "20", NULL, 0.0f, NULL };
...
PRECACHE_MODEL("models/billflx/v_ak47.mdl");
...
{
return DefaultDeploy("models/billflx/v_ak47.mdl", "models/p_ak47.mdl", mp5k_DRAW, "rifle", 0);
}
else
{
return DefaultDeploy("models/billflx/v_ak47.mdl", "models/p_ak47.mdl", mp5k_UNSIL_DRAW, "rifle", 0);
}

}


void CAK47::QuickDeploy()
{
if (m_iWeaponState & WPNSTATE_M4A1_SILENCED)
...
void CAK47::detach(void)
{

SendWeaponAnim(mp5k_DETACH_SILENCER, 0);
}

void CAK47::attach(void)
{
...
void CAK47::PrimaryAttack(void)
{
	if (m_iWeaponState & WPNSTATE_M4A1_SILENCED)
	{
		AK47Fire((0.0) * m_flAccuracy, 0.0825, FALSE);
	}
	else
	{
		AK47Fire2((0.0) * m_flAccuracy, 0.0825, FALSE);
	}
...
cvar_t *weapon_damage_ak47 = NULL;

float CAK47::DamageOptionAK() const
{
    float flDamage;

#ifndef CLIENT_DLL
    flDamage = (float)CVAR_GET_FLOAT("weapon_damage_ak47");
#endif
...
//Vector vecSrc = m_pPlayer->GetGunPosition();
EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/ak47-1.wav", VOL_NORM, ATTN_NORM);
...
void CAK47::AK47Fire2(float flSpread, float flCycleTime, BOOL fUseAutoAim)
{
	m_bDelayFire = true;
	m_iShotsFired++;
	m_flAccuracy = ((float)(m_iShotsFired * m_iShotsFired * m_iShotsFired) / 220) + 0.3;
