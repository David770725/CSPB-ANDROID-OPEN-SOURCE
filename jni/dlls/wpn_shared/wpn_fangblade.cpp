...
LINK_ENTITY_TO_CLASS(weapon_fangblade,CFangblade)
...
void CFangblade::Precache(void)
{
	
PRECACHE_MODEL("models/billflx/v_fangblade.mdl");
...
PRECACHE_SOUND("weapons/fangblade_hit_stab_1.wav");

PRECACHE_SOUND("weapons/fangblade_hit_slash_1.wav");


	m_usFangblade = PRECACHE_EVENT(1, "events/fangblade.sc");
}

int CFangblade::GetItemInfo(ItemInfo *p)
{
...
BOOL CFangblade::Deploy(void)
{
	m_fMaxSpeed = 250;
	m_iSwing = 0;
		
return DefaultDeploy("models/billflx/v_fangblade.mdl", "models/p_fangblade.mdl", KNIFE_DRAW, "kukri", 0);

}


void CFangblade::QuickDeploy()
{
...
void CFangblade::Holster(int skiplocal)
{
	/*m_pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + 4;*/
SetThink(nullptr);
}

void CFangblade::WeaponAnimation(int iAnimation)
{
	int flags;
	...
	PLAYBACK_EVENT_FULL(flags, ENT(m_pPlayer->pev), m_usFangblade, 0, (float *)&g_vecZero, (float *)&g_vecZero, 0, 0, iAnimation, 2, 3, 4);
}

void FindHullIntersectionFangblade(const Vector &vecSrc, TraceResult &tr, float *pflMins, float *pfkMaxs, edict_t *pEntity)
{
	TraceResult trTemp;
...
void CFangblade::PrimaryAttack(void)
{
	...
	SetThink(&CFangblade::DelayedPrimaryAttack);
...
void CFangblade::DelayedPrimaryAttack()
{
...
void CFangblade::SetPlayerShieldAnim(void)
{
	;
}

void CFangblade::ResetPlayerShieldAnim(void)
{
	;
}

bool CFangblade::ShieldSecondaryFire(int up_anim, int down_anim)
{
	;
}

void CFangblade::SecondaryAttack(void)
{
	#ifndef CLIENT_DLL
	...
	SetThink(&CFangblade::DelayedSecondaryAttack);
...
void CFangblade::DelayedSecondaryAttack()
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
	...
	if (tr.flFraction >= 1)
	{
		
		{
			...
			m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + 0.92;
	...
	else
	{
		...
		if (pEntity)
		{
			if (pEntity->Classify() != CLASS_NONE && pEntity->Classify() != CLASS_MACHINE)
			{
				EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/fangblade_hit_stab_1.wav", VOL_NORM, ATTN_NORM);
				m_pPlayer->m_iWeaponVolume = KNIFE_BODYHIT_VOLUME;
...
void CFangblade::Smack(void)
{
	;
}

void CFangblade::SwingAgain(void)
{
	Swing(FALSE);
}

void CFangblade::WeaponIdle(void)
{
	ResetEmptySound();
...
int CFangblade::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
...
int CFangblade::Stab(int fFirst)
{
	;
}
