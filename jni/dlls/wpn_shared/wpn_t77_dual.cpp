...
LINK_ENTITY_TO_CLASS(weapon_t77_dual, CT77_dual)

void CT77_dual::Spawn(void)
{
	pev->classname = MAKE_STRING("weapon_t77_dual");
...
#ifndef CLIENT_DLL
void CT77_dual::AttachToPlayer(CBasePlayer *pPlayer)
{
	...
	while (!m_pLink)
	{
		for (int i = 0; i < MAX_ITEM_TYPES; ++i)
		{
			...
			while (pItem != NULL)
			{
				if (!Q_stricmp(pItem->pszName(), "weapon_t77"))
				{
					m_pLink = pItem;
		...
		if (!m_pLink)
			pPlayer->GiveNamedItem("weapon_t77");
...
void CT77_dual::Precache(void)
{
	
PRECACHE_MODEL("models/billflx/v_t77_2.mdl");

	PRECACHE_MODEL("models/p_t77_2.mdl");
...
int CT77_dual::GetItemInfo(ItemInfo *p)
{
...
BOOL CT77_dual::Deploy(void)
{
	...
	if (m_iSwing)
	{
 DefaultDeploy("models/billflx/v_t77_2.mdl", "models/p_t77_2.mdl", SCARH_DRAW, "dualpistols", 0);
	}
...
void CT77_dual::PrimaryAttack(void)
{
...
void CT77_dual::Kriss_sv_dualFire(float flSpread, float flCycleTime, BOOL fUseAutoAim)
{
	m_bDelayFire = true;
	m_iShotsFired++;
...
float CT77_dual::GetDamage()
{
	return 39;
}


void CT77_dual::WeaponIdle(void)
{
	ResetEmptySound();
