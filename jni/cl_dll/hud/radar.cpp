...
struct CHudRadar::impl_t2 : THudSubDispatcher<CHudRadarLegacy, CHudRadarModernBoard> {};
...
void CHudRadar::Reset()
{
	...
	pimpl2->for_each(&IBaseHudSub::Reset);
}

void CHudRadar::InitHUDData(void)
{
	pimpl->for_each(&IBaseHudSub::InitHUDData);
	pimpl2->for_each(&IBaseHudSub::InitHUDData);
}
...
int CHudRadar::VidInit(void)
{
	...
	pimpl2->for_each(&IBaseHudSub::VidInit);
	return 1;
}
...
int CHudRadar::Draw(float time)
{
	...
	auto& modernScore = pimpl2->get<CHudRadarModernBoard>();
	if (modern.Available())
	{	
		modern.Draw(time);	
		modernScore.Draw(time);
...
void CHudRadar::Think(void)
{
	...
	pimpl2->for_each(&IBaseHudSub::Think);
}
