...
int DrawUtils::DrawHudNumber3( int x, int y, bool DrawZero, int iDigits, int iNumber, int r, int g, int b )
{
	int iWidth = gHUD.GetSpriteRect( gHUD.m_HUD_number_small_0 ).right - gHUD.GetSpriteRect( gHUD.m_HUD_number_small_0 ).left;
	x += ( iDigits - 1 ) * iWidth;
	...
	do
	{
		int k = iNumber % 10;
		iNumber /= 10;
		SPR_Set( gHUD.GetSprite( gHUD.m_HUD_number_small_0 + k ), r, g, b );
		SPR_DrawAdditive( 0, x, y, &gHUD.GetSpriteRect( gHUD.m_HUD_number_small_0 + k ) );
		x -= iWidth;
		iDigits--;
	} while ( iNumber > 0 || ( iDigits > 0 && DrawZero ) );
...
int DrawUtils::DrawHudRound( int x, int y, bool DrawZero, int iDigits, int iNumber, int r, int g, int b )
{
	int iWidth = gHUD.GetSpriteRect( gHUD.m_HUD_round_0 ).right - gHUD.GetSpriteRect( gHUD.m_HUD_round_0 ).left;
	x += ( iDigits - 1 ) * iWidth;
	...
	do
	{
		int k = iNumber % 10;
		iNumber /= 10;
		SPR_Set( gHUD.GetSprite( gHUD.m_HUD_round_0 + k ), r, g, b );
		SPR_DrawAdditive( 0, x, y, &gHUD.GetSpriteRect( gHUD.m_HUD_round_0 + k ) );
		x -= iWidth;
		iDigits--;
	} while ( iNumber > 0 || ( iDigits > 0 && DrawZero ) );
...
int DrawUtils::HudStringLen( const char *szIt, float scale )
{
	...
	{
		if( szIt[0] == '\\' && szIt[4] != '\n' &&
			(szIt[4] == 'y' || szIt[4] == 'w' || szIt[4] == 'd' || szIt[4] == 'R') ) // not sure is reversing handled correctly
		{
			szIt++;
			continue;
		}
