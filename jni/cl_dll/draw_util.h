...
public:
	...
	static int DrawHudNumber3( int x, int y, bool DrawZero, int iDigits, int iNumber,
						   int r, int g, int b);
	...
	{
		if (!g_iXash || gHUD.hud_textmode->value )
			*height = 23, *width = HudStringLen( (char *)szIt );
		else
						   ...
						   bool drawStroke = true )
	{
		...
		if ( drawStroke )
		{
			...
			DrawOutlinedRect(x, y, wide, tall, 200, 140, 0, 200);
		}
	}
	...
	static inline void DrawOutlinedRect2(int x, int y, int wide, int tall,
		int r = 0, int g = 0, int b = 0, int a = 255)
	{
		FillRGBABlend(x + 1, y, wide - 1, 1, r, g, b, a);
		FillRGBABlend(x, y, 1, tall - 1, r, g, b, a);
		FillRGBABlend(x + wide - 1, y + 1, 1, tall - 1, r, g, b, a);
		FillRGBABlend(x, y + tall - 40, wide - 1, 40, r, g, b, a);
	}
	...
	static int DrawHudRound( int x, int y, bool DrawZero, int iDigits, int iNumber,
						   int r, int g, int b);
